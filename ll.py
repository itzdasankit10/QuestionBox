...
async def _run_llm_call(model_name: str, prompt: str, max_tokens: int = 1024) -> str:
    ...
    if model_name.startswith("gpt-"):
        ...
        if "gemma" in model_name.lower() or "llama" in model_name.lower() or "mistral" in model_name.lower():
            ...
            e = await ollama_client.generate(model=model_name, prompt=d, stream=False, options=b)
        else:
            e = await ollama_client.generate(model=model_name, prompt=a, stream=False, options=b)

        t = e.get("response", "").strip()
    return t
